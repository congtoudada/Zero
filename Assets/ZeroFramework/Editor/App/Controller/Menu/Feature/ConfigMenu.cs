...
namespace Zero.Editor
{
    public class ConfigMenu : ZeroSubMenu
    {
        ...
        public void CreateConfigKey()
        {
            ...
            try
            {
                ...
                if (YamlPath.EndsWith(".yaml") || YamlPath.EndsWith(".yml"))
                {
                    ...
                    if (!CheckFileExist(absolutePath)) return;
                ...
                if (configKit.Keys == null)
                {
                    Debug.LogError("ConfigKey is Null!");
                    return;
                }
                WriteScript(outputPath, configKit.Keys);
                SaveCache();
                AssetDatabase.Refresh();
                EditorUtility.RevealInFinder(Relative2Absolute(OutputFolder));
        ...
        private void WriteScript(string path, List<string> keys)
        {
            ...
            var charArray = outputFilename.ToCharArray();
            ...
            ScriptInfo scriptInfo = new ScriptInfo()
            {
                ...
                ClassName = new string(charArray),
