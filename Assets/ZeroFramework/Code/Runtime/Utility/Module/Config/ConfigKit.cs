...
namespace Zero.Utility
{
    public class ConfigKit : IUtility, IConfigKit
    {
        ...
        public ConfigInfo CreateConfigInfo(string location, ConfigInfo.FileType fileType, ConfigInfo.LoadType loadType,
            ConfigInfo.OrderType orderType = ConfigInfo.OrderType.BEFORE_INCLUDE)
        {
            return new ConfigInfo(location, fileType, loadType, orderType);
        }

        public IConfigKit Equip(ConfigInfo configInfo)
        {
            switch (configInfo.fileType)
            {
                case ConfigInfo.FileType.SCRIPTABLE:
                    _G = new ScriptableConfigHelper().Resolve(configInfo)?.OverrideConfig(_G);
                    break;
                case ConfigInfo.FileType.YAML:
                    _G = new YamlConfigHelper().Resolve(configInfo)?.OverrideConfig(_G);
                    break;
                case ConfigInfo.FileType.JSON:
