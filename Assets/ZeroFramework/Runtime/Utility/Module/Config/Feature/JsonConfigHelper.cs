...
{
    public class JsonConfigHelper : BaseConfigHelper
    {
        public override JObject Resolve(ConfigInfo configInfo)
        {
            base.Resolve(configInfo);
            if (configInfo.fileType != ConfigInfo.FileType.JSON) return null;
            if (configInfo.loadType == ConfigInfo.LoadType.UNITY_WEB_REQUEST)
            {
                logger.Error("ScriptableObject配置不支持使用UnityWebRequest加载！");
                return null;
            }
